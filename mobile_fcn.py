...
# --- 2. Robust Data Fetching (Fix for $100 Error) ---
...
def get_asset_info_robust(symbol):
    try:
        ...
        return {"name": symbol, "curr": 100.0, "pe": "N/A"}
...
# --- 3. Strategy Settings (No KI Switch Included) ---
with st.container():
    ...
    if no_ki_mode:
        ki_pct = 0.0
        st.info("No KI Mode: Win rate will be based on price remaining above $0.")
    else:
...
# --- 5. Monte Carlo Simulation (500 Paths with Fat Tail) ---
n_days, n_paths, dt, mu = 180, 500, 1/252, 0.05
paths = np.ones((n_days, n_paths))

for i in range(1, n_days):
    # 混合 Student-t 分佈模擬真實市場肥尾風險
...
# --- 6. Plotting (Limited to 150 paths for UI speed) ---
fig = go.Figure()
...
st.markdown(f"""
    <div style="background-color: #1E1E1E; padding: 20px; border-radius: 15px; border: 2px solid #00FFA3; text-align: center;">
        ...
        <p style="color: #888888; font-size: 12px;">Definition: Probability of NO KI event during 180 days.</p>
        <p style="color: #888888; font-size: 10px;">(500 Paths Simulated)</p>
    </div>
    """, unsafe_allow_html=True)
